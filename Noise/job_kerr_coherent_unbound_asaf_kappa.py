...
def save(output, simulation_parameters):
    ...
    import pickle 
    name = 'adpd_kerr_coherent_N_threshold_unbound_asaf_kappa_d=5_v3'
...
def optimization(simulation_parameters, phi, delta, phi_delta, conv_tol, options):
    ...
    with open('params_p_qfi_cfi_kerr_coherent_N_threshold_unbound_layers=5.p', 'rb') as fp:
    ...
    with open('params_m_qfi_cfi_kerr_coherent_N_threshold_unbound_layers=5.p', 'rb') as fp:
    ...
    for kappa in kappa_list:
        ...
        # Losses
        rho = sp.linalg.expm_multiply(kappa*setup.L_Kerr_ad(), rho)
        grad_rho = sp.linalg.expm_multiply(kappa*setup.L_Kerr_ad(), grad_rho)
        rho = sp.linalg.expm_multiply(kappa*setup.L_Kerr_pd(), rho)
        grad_rho = sp.linalg.expm_multiply(kappa*setup.L_Kerr_pd(), grad_rho)
